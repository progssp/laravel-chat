            ...
            if(d.sender_id != user.id){
                if(this.state.active_user.length != 0){
                    if(this.state.active_user[0].id == d.sender_id){
                        ...
                        alert('you got a new message : '+d.message);
                    }
                }
                else{
                    alert('no ative user, you got a new message : '+d.message);
                }
            }
    ...
    render(){
        ...
        return (
            <div className="container">     
                <div className="row">
                    <div className="col">
                        <button onClick={this.getActiveUser}>get active user</button>
                    </div>
                </div>           
                        ...
                        <div className="card">
                            ...
                            <div className="card-body">
                                <ul id="user_list" className="user_list list-group">
                                    ...
                                    <a href="#"><li id={"user_"+number.id} onClick={this.loadChats} className="list-group-item list-group-item-action" key={number.id}>{number.name}</li></a>  )}
                                </ul>
                            </div>                            
                        </div>
                    </div>
                            ...
                            <div className="card-body">
                                <ul id="chat_list" className="chat_list list-group">
                                    {this.state.msg_list.map((msgs) =>
                                        <li className="msg_style_for_rec" id={msgs.id} key={msgs.id}>{msgs.message}</li>
                                    )}
                                </ul>
                            </div>
